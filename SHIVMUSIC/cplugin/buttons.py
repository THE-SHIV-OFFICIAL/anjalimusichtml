...
# 🎨 Stable button palette: action meaning stays consistent between refreshes.
def get_style_map():
    return {
...
def community_buttons():
    """Reusable support/update row for clone playback messages."""
    return [
...
def panel_markup_clone(_, vidid, chat_id, bot_username=None):
    ...
    buttons = [
        [
            add_me_button(bot_username or app.username, s_map[1]),
        ],
        [
            create_btn(text="▷", callback_data=f"ADMIN Resume|{chat_id}", style=s_map[3], no_emoji=True),
            create_btn(text="II", callback_data=f"ADMIN Pause|{chat_id}", style=s_map[3], no_emoji=True),
