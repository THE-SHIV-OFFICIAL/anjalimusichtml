...
# 🎨 Stable button palette: playback actions should not change color randomly.
def get_style_map():
    return {
...
# Helper for setup/add-to-group actions
def setup_button(style):
    return create_btn(
        text="⚙ sєᴛᴜᴘ / ᴧᴅᴅ ᴍє",
...
def community_markup():
    """HTML-friendly support and updates actions for playback cards."""
    return [
        [
            create_btn(text="✦ sᴜᴘᴘᴏʀᴛ", url=config.SUPPORT_CHAT, style=ButtonStyle.SUCCESS),
            create_btn(text="✦ ᴜᴘᴅᴀᴛᴇs", url=config.SUPPORT_CHANNEL, style=ButtonStyle.PRIMARY),
        ]
    ]
