...
async def skip(cli, message: Message, _, chat_id):
    ...
    if not check:
        return await message.reply_text(_["queue_2"], parse_mode=ParseMode.HTML)
        
    # Loop on/off check
    loop = await get_loop(chat_id)
    if loop != 0:
        return await message.reply_text(_["admin_8"], parse_mode=ParseMode.HTML)
        ...
        if state.isnumeric():
            ...
            if count > 2:
                ...
                if 1 <= state <= count:
                    ...
                    return await message.reply_text(_["admin_11"].format(count), parse_mode=ParseMode.HTML)
            else:
                return await message.reply_text(_["admin_10"], parse_mode=ParseMode.HTML)
        else:
                return await message.reply_text(_["admin_11"].format(len(check)-1), parse_mode=ParseMode.HTML)
